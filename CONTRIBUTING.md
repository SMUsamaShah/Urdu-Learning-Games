...
- `src/lib/scenery.js` — the sky, sun, clouds, hills and ground every screen
  sits on.
- `src/lib/plant.js` — the garden at the left of every game: the pot that grows,
  and the row of finished trees behind it.
...
**The garden is the progress.** It stands where the spider used to on every game
screen, and there is no separate score anywhere: one right answer is one pour of
water, a full-grown tree is a level, and the row behind the pot is every tree
grown so far. `addStage()` puts it in place, and a scene talks to it as
`this.plant` — `wonder()` for a wrong answer, `cheer()` for finishing something.
A scene that wants it somewhere else passes `plant: { x, y, height }`.

`npm run preview-plant` draws every growth step and all six seeds onto a sheet.
Use it: whether a plant looks like a plant is not something a test can answer,
and the alternative is playing to level nine to find out what level nine looks
like.
