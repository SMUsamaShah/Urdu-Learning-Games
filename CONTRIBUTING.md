...
**The tiles carry real Urdu, and the model did not draw it.** `npm run tiles`
generates a picture per menu tile, and the briefs deliberately ask for blank
places — an unmarked balloon, an empty card, a plain wooden disc — because an
image model cannot draw Nastaliq and will put English on the tile of an Urdu app
if given the chance. `make-tile-art.mjs` then paints the letters in itself, out
of `content/glyphs.json`, in the same Playwright page it already uses to crop
and round. A brief is `{art, slots}`; a slot is `{letter | number, form, x, y,
height}` in fractions of the tile.

Two things a slot has to respect. It belongs in the **upper two thirds** — the
bottom third carries the game's name at runtime and anything under there is
invisible — and it wants to be **large**: a letter at a tenth of the tile is
seventeen pixels on a phone and reads as a highlight rather than a letter. Two
big letters beat five small ones, which is why several briefs were rewritten to
ask for fewer, larger objects. Moving a slot is free; the raw generations are
cached in `.image-cache/tile/` and only a changed brief costs anything.
