...
export function create(scene, { width, height }) {
  ...
  // The crown starts below the top of the trunk.
...
/* A point on a phyllotactic spiral inside the crown. */
  ...
  const slip = (k) => {
    ...
    // True immediately, even though the leaves are still falling: what is published is what the tree now *is*.
    shown = Math.max(0, Math.min(steps, k));
    ...
    scene.time.delayedCall(1400, () => {
      ...
      scene.tweens.add({
        ...
        onComplete: () => {
          ...
          // The leaves fall on the way to next year, which is the one moment the tree is allowed to look like it lost something.
