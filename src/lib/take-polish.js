/* Tidies a recording after it has been made: takes the room off, trims the silence, evens the level. */

import { dereverb } from './dereverb.js';

/* Frame length for the level envelope. */
const WINDOW_MS = 20;
...
/* The gate separates speech from the measured noise floor. */
...
/* Kept either side of the speech. */
...
/* The take with its reverberation suppressed, or null if there was none worth removing. */
...
function findSpeech(buffer) {
  ...
  // Nothing to separate: either the take is all room, or it is speech from the first sample to the last.
  ...
  // The level the voice is actually at, read over the speech only.
  const speech = [];
...
function trimAndLevel(ctx, buffer, bounds) {
  ...
  // The loudest sample in what is being kept.
  let peak = 0;
  ...
  // Four limits, and the take gets the smallest.
  const gain = Math.max(
    1,
    Math.min(
  ...
  for (let c = 0; c < buffer.numberOfChannels; c++) {
    ...
    for (let i = 0; i < length; i++) {
      ...
      // One-pole high-pass in the sample domain.
...
function encode(ctx, buffer, mime) {
  return new Promise((resolve) => {
    ...
    try {
      ...
      // Stopped by the source ending rather than by a timer, so a slow frame cannot cut the tail off.
      source.onended = () => recorder.state === 'recording' && recorder.stop();
      recorder.start();
      source.start();

      // Stop the recorder even if the context is interrupted.
      setTimeout(
...
export async function polishTake(ctx, blob, mime) {
  ...
  try {
    ...
    // Remove room noise before trimming the take.
