...
// 1. Open a game and return with Back.
...
// Both halves: Home up *and* the game gone.
...
// 3. Paging the menu must not add history entries.
...
await settled();
// The gate settles its promise from inside its own unwind, so by the time settings opens the gate's entry is gone.
