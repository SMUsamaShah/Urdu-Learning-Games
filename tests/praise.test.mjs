...
describe('praise', () => {
  test('there are several, each named once', () => {
    // Keep enough phrases to avoid immediate repetition.
  ...
  test('every phrase has a roman and an English gloss', () => {
    // The roman is what the recorder prints; the English is what the ribbon shows the parent.
    for (const praise of PRAISE) {
  ...
  test('the ribbon knows how to measure them', () => {
    // The ribbon picks one em across every string it can ever show, and wellDone() puts these on it.
